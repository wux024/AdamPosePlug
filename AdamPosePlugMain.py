...
import sys
from AdamPosePlugUI import Ui_PoseAnalysis
import pandas as pd
import cv2
import yaml
import os
...
class Figure_Canvas(FigureCanvas):
    def __init__(self, width=20.5, height=5.62, dpi=80):
        self.fig = Figure(figsize=(width, height), dpi=dpi)
        super(Figure_Canvas, self).__init__(self.fig)
        self.ax = self.fig.add_subplot(1, 1, 1, projection='3d')

class MyMainWindow(QMainWindow, Ui_PoseAnalysis):
    def __init__(self):
    ...
    def pause_to_continue(self):
        ...
        if self.pause_flag:
            ...
            self.Pause.setText(_translate("PoseAnalysis", "Continue"))
        else:
            self.timer.start()
            self.pause_flag = True
    ...
    def create_keypoints_selection(self):
        ...
        for keypoint_id, keypoint in enumerate(self.key_points):
            ...
            exec('self.keypoint%s.setText(_translate("PoseAnalysis", "%s"))' % (keypoint_id, keypoint))
    ...
    def angles_process(self):
        ...
        angles = compute_angles(vecs)*180.0/np.pi
    ...
    def figure_create(self):
        ...
        self.PoseEstimation3DVideo = Figure_Canvas()
